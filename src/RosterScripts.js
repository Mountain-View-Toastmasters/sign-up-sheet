...
function getLastRowOfRoster(spreadsheet) {
  var firstNameCol = spreadsheet.getRange(`B${ROSTER_HEADER_ROW+1}:B`).getValues();
  return firstNameCol.filter(String).length + ROSTER_HEADER_ROW;
}

function getRosterSheetData() {
  // Open MVTM's Roster Spreadsheet
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(ROSTER_SHEET_NAME);
  ...
  return sheet.getRange(
    ...
    ROSTER_END_COL - ROSTER_START_COL + 1).getValues();
}
