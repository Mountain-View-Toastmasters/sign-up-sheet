...
function advanceSignUpSheet() {
  var signUpSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Sign-up Sheet - Automatic Advance")
  var signUpTemplate = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("SignUp Template");
...
function clearOutCurrentSignUp(signUpSheet, signUpTemplate) {
  signUpSheet.getRange( __SIGNUP_SHEET_HEADER_ROW_NUM, 
                        __SIGNUP_SHEET_SECTION_START_COL_NUM, 
                        signUpTemplate.getLastRow(), 
                        signUpTemplate.getLastColumn() + 2) // with 2 header cols
              .setValue("")
}
...
function insertNewSignupSection(signUpSheet, signUpTemplate) {
  var signUpTemplateRange = signUpTemplate.getRange(1, 1, signUpTemplate.getLastRow(), signUpTemplate.getLastColumn());
  ...
  signUpSheet.getRange(7, signUpSheet.getLastColumn() + 1, rangeVals.length, signUpTemplate.getLastColumn() )
    .setValues(rangeVals)
...
function updateDate(signUpSheet, signUpTemplate) {
  var dateCell = signUpSheet.getRange(__SIGNUP_SHEET_HEADER_ROW_NUM, signUpSheet.getLastColumn() - 4)
  var prevDate = signUpSheet.getRange(__SIGNUP_SHEET_HEADER_ROW_NUM, signUpSheet.getLastColumn() - 4 - signUpTemplate.getLastColumn()).getValue();
