...
class ToastmasterDetails {
  constructor() {
    ...
    this.wordOfTheDay = toastmasterDetailsData[WORD_OF_THE_DAY_ROW][DETAILS_COL];
    this.wordOfTheDayPartOfSpeech = toastmasterDetailsData[WOTD_PART_OF_SPEECH_ROW][DETAILS_COL];
    this.wordOfTheDayDescription = toastmasterDetailsData[WOTD_DESCRIPTION][DETAILS_COL];
    this.wordOfTheDaySampleSentence = toastmasterDetailsData[WORD_SAMPLE_SENTENCE][DETAILS_COL];
  ...
  populateRolesSheet(rolesSheet, roleEntryRow) {
    ...
    setCell(Word_of_the_Day_Sample_Sentence_COL, this.wordOfTheDaySampleSentence);
...
class SpeakerDetails {
  constructor(toastmasterDetailsData, speakerRow) {
    ...
    this.minSpeechTime = toastmasterDetailsData[speakerRow][MIN_SPEECH_TIME_COL];
    this.maxSpeechTime = toastmasterDetailsData[speakerRow][MAX_SPEECH_TIME_COL];
  }

  populateRolesSheet(rolesSheet, roleEntryRow, speakerCol) {
    // Refilling fields that the sign up sheet probably hit jic
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_COL_OFFSET).setValue(this._name);
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Pathway_COL_OFFSET).setValue(this._pathway);
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Level_COL_OFFSET).setValue(this._level);
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Project_COL_OFFSET).setValue(this._project);

    // Filling out the TM Details-unique fields
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Speech_Title_COL_OFFSET).setValue(this.speechTitle);
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Min_Time_COL_OFFSET).setValue(this.minSpeechTime);
    rolesSheet.getRange(roleEntryRow, speakerCol + Speaker_Max_Time_COL_OFFSET).setValue(this.maxSpeechTime); 
  }  
}

function getToastmastersDetailsSheetData() {
  // Open MVTM's Toastmasters Details Spreadsheet
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(TM_DETAILS_SHEET_NAME);
  // Get the values into the data Array (ignoring the first few header rows)
  return sheet.getRange(
    TMDETAILS_START_ROW,
    TMDETAILS_START_COL,
    TMDETAILS_END_ROW - TMDETAILS_START_ROW + 1,
    TMDETAILS_END_COL - TMDETAILS_START_COL + 1).getValues();
}
...
function copyToastmasterDetailsToRoles() {
  ...
  const roleEntryRow = getOrCreateRoleEntryRow(prettyFormatDate(tmDetails.date));
...
function clearToastmasterDetails() {
  var tmDetailsSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(TM_DETAILS_SHEET_NAME);
